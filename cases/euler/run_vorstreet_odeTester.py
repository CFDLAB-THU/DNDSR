...
def run():
    ...
    dt_opts = collections.defaultdict(
        lambda: [1 / 10, 1 / 16, 1 / 20, 1 / 25, 1 / 40, 1 / 50] + [1 / 80, 1 / 100]
    )
    dt_opts["BDF2"] = (
        [1 / 10, 1 / 16, 1 / 20, 1 / 25, 1 / 40, 1 / 50]
        + [1 / 80, 1 / 100]
        + [
            1 / 64,
            1 / 100,
    ...
    # dt_opts = collections.defaultdict(lambda: [1 / 10, 1 / 16, 1 / 25, 1 / 50])
    # dt_opts["BDF2"] = [1 / 10, 1 / 16, 1 / 25, 1 / 50] + [
    #     1 / 64,
    #     1 / 100,
    #     1 / 200,
    # ]
    ...
    global_opt = {
        ...
        "/timeMarchControl/tEnd": 1.0,
        "/convergenceControl/rhsThresholdInternal": 1e-4,
        "/convergenceControl/nTimeStepInternalMin": 10,
        "/restartState/lastRestartFile": "../data/outCylinderB1/CylinderB1_T0Ref/out_ref_esdirk4__1000_p8.restart.dnds.h5",
        "/implicitReconstructionControl/recLinearScheme": 0,
        "/implicitReconstructionControl/nInternalRecStep": 5,
        # O2
        # "/implicitReconstructionControl/nInternalRecStep": 1,
        # "/vfvSettings/maxOrder": 1,
        # "/vfvSettings/intOrder": 1,
        # O2 end
    ...
    try:
        for name, options in options_list.items():
            ...
            envC.update(
                {
                    "DNDS_DIST_OMP_NUM_THREADS": f"{nThread}",
                    "OMP_NUM_THREADS": f"{nThread}",
                }
            )
