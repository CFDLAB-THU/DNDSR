...
namespace DNDS
{
    void pybind11_bind_ArrayEigenMatrix_All(py::module_ &m)
    {
        ...
        pybind11_callBindArrayEigenMatrixs_rowsizes<DynamicSize>(m); // N x 8
        ...
        pybind11_ArrayEigenMatrixPair_define<DynamicSize, DynamicSize>(m);
        // ! not using NonUniformSize-d ArrayEigenMatrixPair for now
        // pybind11_ArrayEigenMatrixPair_define<NonUniformSize, NonUniformSize>(m);
    }
}