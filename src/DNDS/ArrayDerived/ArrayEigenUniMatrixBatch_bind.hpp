...
namespace DNDS
{
    ...
    void pybind11_ArrayEigenUniMatrixBatch_define(py::module_ &m)
    {
        ...
        ArrayEigenUniMatrixBatch_ // the once for all resize
            .def("Resize", [](TArrayEigenUniMatrixBatch &self, index size, int r, int c, py::array_t<int, pybind11::array::c_style | pybind11::array::forcecast> rowsizes)
                 { return self.Resize(size, r, c, [&](index i)
                                      { return rowsizes.at(i); }); }, py::arg("size"), py::arg("r"), py::arg("c"), py::arg("rowsizes"));
        ArrayEigenUniMatrixBatch_
